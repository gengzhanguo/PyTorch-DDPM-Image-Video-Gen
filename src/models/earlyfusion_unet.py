...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channel, out_channel, *, t_emb_dim=None, dims=2, dropout=0.1, scale_shift=True, conv_skip=False):
        ...
        self.t_emb_mlp = nn.Sequential(
            nn.SiLU(),
            nn.Linear(t_emb_dim, out_channel*2 if scale_shift else out_channel) # 2*out_channel as scale and shift
        ) if exists(t_emb_dim) else None
    ...
    def forward(self, x, t_emb=None):
        ...
        if exists(self.t_emb_mlp) and exists(t_emb):
            t_emb = self.t_emb_mlp(t_emb)
            while t_emb.dim() < h.dim():
                t_emb = t_emb[..., None]
            if self.scale_shift:
                out_norm, out_rest = self.block_out[0], self.block_out[1:]
                scale, shift = t_emb.chunk(2, dim=1) # split the 2*out_channel as scale and shift, shape: (bs, ct/2, 1, 1)
    ...
    def __init__(self, args):
        ...
        for i, ((in_channel, out_channel), full_attn, num_head, dim_head) in enumerate(zip(self.channel_dims, full_attns, num_heads, dim_heads)):
            ...
            self.down_layers.append(nn.ModuleList([
                ResidualBlock(in_channel, in_channel, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv), # skip connection by residual block
                ResidualBlock(in_channel, in_channel, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv), # skip connection by residual block
    ...
    def __init__(self, args):
        ...
        for i, ((in_channel, out_channel), full_attn, num_head, dim_head) in enumerate(zip(*map(reversed, (channel_dims, full_attns, num_heads, dim_heads)))):
            ...
            self.up_layers.append(nn.ModuleList([
                ResidualBlock(in_channel+out_channel, out_channel, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv),
                ResidualBlock(in_channel+out_channel, out_channel, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv),
...
class BottleneckBlock(nn.Module):
    def __init__(self, args):
        ...
        self.res1 = ResidualBlock(self.bottleneck_dim, self.bottleneck_dim, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv)
        self.att = FullAttentionBlock(args.dim_conv, self.bottleneck_dim, num_head=self.args.num_head, dim_head=self.args.dim_head)
        self.res2 = ResidualBlock(self.bottleneck_dim, self.bottleneck_dim, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv)
    def forward(self, x, t):
        x = self.res1(x, t)
        x = self.att(x)
        x = self.res2(x, t)
...
class EarlyFusion_UNet(nn.Module):
    def __init__(self, args, self_cond=False, learn_var=False):
        super(EarlyFusion_UNet, self).__init__()
        self.args = args
        ...
        init_channel = (self.args.image_channel*2 + self.args.latent_dim) if self_cond else (self.args.image_channel + self.args.latent_dim)
        ...
        self.final_res = ResidualBlock(self.args.base_channel*2, self.args.base_channel, t_emb_dim=self.args.t_emb_dim, dims=args.dim_conv)
    ...
    def forward(self, x, condition, t, x_self_cond=None):
        ...
        if len(orig_shape) == 5:
            ...
            if self.args.dim_conv == 2:
                ...
                # Reshape the condition to (batch_size, 1, 1, latent_dim) and expand it
                condition = condition.unsqueeze(2).unsqueeze(3)  # shape: [B, latent_dim] -> [B, latent_dim, 1, 1, ]
                condition = condition.expand(-1, -1, H, W)  # shape: [B, latent_dim, 1, 1] -> [B, latent_dim, 160, 160]
                x = torch.cat([x, condition], dim=1)  # Concatenate condition: shape [B, 9+2+8, 160, 160] -> [B, 19, 160, 160]
            elif self.args.dim_conv == 3:
                B, C, D, H, W = x.shape
                condition = condition.unsqueeze(2).unsqueeze(3).unsqueeze(4)  # Shape: [B, latent_dim] -> [B, latent_dim, 1, 1, 1]
                condition = condition.expand(-1, -1, D, H, W)  # Expand to match [B, latent_dim, D, H, W]
                # Concatenate condition to input along the channel dimension
                x = torch.cat([x, condition], dim=1)  # Shape: [B, C+latent_dim, D, H, W]
            else:

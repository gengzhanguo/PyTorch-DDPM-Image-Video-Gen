    ...
    def __init__(self, args):
        ...
        for i, ((in_channel, out_channel), full_attn, num_head, dim_head) in enumerate(zip(self.channel_dims, full_attns, num_heads, dim_heads)):
            ...
            self.down_layers.append(
                nn.ModuleList([
                    CondResBlock(in_channel, in_channel, t_emb_dim=self.args.t_emb_dim, c_emb_dim=self.args.c_emb_dim, dims=args.dim_conv), # skip connection by residual block
                    CondResBlock(in_channel, in_channel, t_emb_dim=self.args.t_emb_dim, c_emb_dim=self.args.c_emb_dim, dims=args.dim_conv), # skip connection by residual block
    ...
    def __init__(self, args):
        ...
        for i, ((in_channel, out_channel), full_attn, num_head, dim_head) in enumerate(zip(*map(reversed, (channel_dims, full_attns, num_heads, dim_heads)))):
            ...
            self.up_layers.append(
                nn.ModuleList([
                    CondResBlock(in_channel+out_channel, out_channel, t_emb_dim=self.args.t_emb_dim, c_emb_dim=self.args.c_emb_dim, dims=args.dim_conv),
                    CondResBlock(in_channel+out_channel, out_channel, t_emb_dim=self.args.t_emb_dim, c_emb_dim=self.args.c_emb_dim, dims=args.dim_conv),
...
class Uniform_UNet(nn.Module):
    ...
    def __init__(self, args, self_cond=False, learn_var=False):
        super(Uniform_UNet, self).__init__()
        self.args = args
